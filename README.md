...
`scripts/bucket-b-prep.sh` reads on-chain supply and Ember Reserve state, enforces the 300M floor
and a per-cycle circuit breaker, and prepares the monthly treasury action (burn or locked-liquidity
add). Read-only compute; all signing happens on hardware wallets — no keys in CI.
Ledger: `state/bucket-b-ledger.csv`.